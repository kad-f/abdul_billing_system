...
class Invoice
{
	...
	{
		$sqlQuery = "
			SELECT id, email, first_name, last_name, address, mobile, role 
	...
	{
		$sqlQuery = "SELECT id, first_name, last_name FROM " . $this->invoiceUserTable . " WHERE role = 'USER'";
		return $this->getData($sqlQuery);
	}

	public function saveInvoice($POST)
	{
		$userId = $POST['companyName'];
		$userInfo = $this->getUserInfo($userId);

		$sqlInsert = "INSERT INTO " . $this->invoiceOrderTable . "(user_id, order_receiver_name, order_receiver_address, order_total_before_tax, order_total_tax, order_tax_per, order_total_after_tax, order_amount_paid, order_total_amount_due, note) VALUES ('" . $userId . "', '" . $userInfo['first_name'] . ' ' . $userInfo['last_name'] . "', '" . $POST['address'] . "', '" . $POST['subTotal'] . "', '" . $POST['taxAmount'] . "', '" . $POST['taxRate'] . "', '" . $POST['totalAftertax'] . "', '" . $POST['amountPaid'] . "', '" . $POST['amountDue'] . "', '" . $POST['notes'] . "')";
		echo $sqlInsert;
	...
	{
		$sqlQuery = "SELECT first_name, last_name FROM " . $this->invoiceUserTable . " WHERE id = '" . $userId . "'";
		$result = mysqli_query($this->dbConnect, $sqlQuery);
	...
	public function getInvoiceList()
	{
		if ($_SESSION['role'] === 'admin') {
			...
			$sqlQuery = "SELECT * FROM " . $this->invoiceOrderTable . " WHERE user_id = '" . $_SESSION['userid'] . "'";
		}
		return  $this->getData($sqlQuery);
	}
